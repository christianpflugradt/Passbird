...
open class Slots<T> : Iterable<MutableOption<T>> {
    ...
    private val items: Array<MutableOption<T>> = Array(10) { mutableOptionOf() }

    operator fun get(index: Int) = requireValidIndex(index).run { items[index] }
    operator fun get(slot: Slot) = get(slot.index())

    operator fun set(index: Int, value: T) = requireValidIndex(index).run { items[index].set(value) }
