...
private const val PASSWORD_TREE_TRASH_MIGRATION_ID = "password-tree-trash"

class PasswordTreeTrashMigrationDetector constructor(
    private val configuration: ReadableConfiguration,
    private val systemOperation: SystemOperation,
) : PreLaunchMigrationDetector {
    override fun detect() = if (migrationRequired()) {
        MigrationRequest(setOf(PendingMigration(PASSWORD_TREE_TRASH_MIGRATION_ID)))
    } else {
        MigrationRequest.empty()
    }

    private fun migrationRequired() = systemOperation.exists(filePath) &&
        systemOperation.readBytesFromFile(filePath).let { bytes -> bytes.isNotEmpty() && isLegacyTrashPasswordTree(bytes) }
...
class PasswordTreeTrashMigration constructor(
    private val migrationAuthenticationService: MigrationAuthenticationService,
    private val passwordTreeTrashMigrationService: PasswordTreeTrashMigrationService,
    private val systemOperation: SystemOperation,
) : Migration {
    override val id = PASSWORD_TREE_TRASH_MIGRATION_ID
    override val order = 4

    override fun run() {
        migrationAuthenticationService.authenticate(maxAttempts = 3)
            .onSuccess { migrationCredentials ->
                passwordTreeTrashMigrationService.migrate(migrationCredentials.keyCopy())
            }
            .onFailure {
