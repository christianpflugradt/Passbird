...
CURRENT_STEP="source-selective-export"
run_session "$CURRENT_STEP" "$SOURCE_HOME" "$MASTER_PASSWORD" "e*" "n" "$EXPORT_PASSWORD" "$EXPORT_PASSWORD" "1" "q"
assert_contains "$LAST_OUTPUT" "Available Nests:"
assert_contains "$LAST_OUTPUT" "Specify Nest Slots separated by ','"
assert_not_contains "$LAST_OUTPUT" "Input 1 to export only selected Nests"
assert_contains "$LAST_OUTPUT" "2 eggs successfully exported."
assert_file_exists "$SOURCE_HOME/passbird-export.zip"
pass "$CURRENT_STEP"
