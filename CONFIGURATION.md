...
| `application.flow.globalHotkey.backend`               | Selects the backend Passbird uses for the temporary global hotkey during multi-step `u[EggId]` flows. Accepted values are `auto`, `win32`, `quartz`, `x11`, and `wayland`. `auto` chooses `win32` on Windows, `quartz` on macOS, and `x11` when an X11 display is available. Explicit values are mainly useful for verification, troubleshooting, or forcing a specific backend on systems where multiple environments may be present. | auto                 |
