...
| `application.flow.globalHotkey.backend`               | Selects the backend Passbird uses for the temporary global hotkey during multi-step `u[EggId]` flows. Accepted values are `auto`, `win32`, `quartz`, `x11`, and `wayland`. `auto` lets Passbird choose the backend that fits the current environment. Explicit values are mainly useful for verification, troubleshooting, or forcing a specific backend on systems where multiple environments may be present. | auto                 |
