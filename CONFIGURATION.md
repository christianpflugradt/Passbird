...
| `application.flow.globalHotkey.enabled`               | Enables or disables the temporary global hotkey used as an alternative to Enter during multi-step `u[EggId]` flows. The flow still works with Enter when the hotkey is disabled or unavailable. | false                |
...
| `application.flow.globalHotkey.backend`               | Selects the backend Passbird uses for the temporary global hotkey during multi-step `u[EggId]` flows. Accepted values are `auto`, `win32`, `carbon`, `quartz`, and `x11`. `auto` chooses `win32` on Windows, `carbon` on macOS, and `x11` when an X11 display is available. Carbon works with macOS secure input and does not require Input Monitoring permission. Explicit values are mainly useful for verification, troubleshooting, or forcing a specific backend on systems where multiple environments may be present. When `quartz` is explicitly selected, Passbird opens the Input Monitoring settings and terminates at startup if the launching terminal app does not have the required macOS permission yet. | auto                 |
