...
[![release](https://img.shields.io/github/v/release/christianpflugradt/Passbird?display_name=tag&sort=semver)](https://github.com/christianpflugradt/Passbird/releases) [![build](https://github.com/christianpflugradt/Passbird/actions/workflows/build.yml/badge.svg?branch=main)](https://github.com/christianpflugradt/Passbird/actions/workflows/build.yml) [![branch coverage](https://img.shields.io/endpoint?url=https://christianpflugradt.github.io/Passbird/coverage/branch-coverage.json)](https://christianpflugradt.github.io/Passbird/coverage/report/) [![instruction coverage](https://img.shields.io/endpoint?url=https://christianpflugradt.github.io/Passbird/coverage/instruction-coverage.json)](https://christianpflugradt.github.io/Passbird/coverage/report/) [![license](https://img.shields.io/badge/License-Apache%202.0-blue.svg)](https://opensource.org/licenses/Apache-2.0)
...
Passbird is a lightweight, terminal-based password manager built in Kotlin. It operates entirely offline, securing your passwords with AES-GCM encryption and storing the encryption key in a PKCS12 Java KeyStore.
...
Import expects every exported Nest record in `passbird-export.json` to declare a unique Slot from `0` to `9`, every exported Nest to contain distinct EggIds, and every exported Protein record to declare a unique Slot from `0` to `9` within its Egg. Exported Protein records must contain both Type and Structure fields or leave both empty. If Nest or Protein slot metadata is missing, duplicated, or out of range, if one exported Nest repeats an EggId, or if an exported Protein record contains only one of Type or Structure, Passbird rejects the file instead of importing it into another Nest or Protein slot.
...
- `./gradlew property` runs Kotest Property tests for persistence, exchange, backup, and crypto invariants.
